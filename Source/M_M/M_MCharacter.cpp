...
{	
	...
	{
		UE_LOG(LogM_M, Error, TEXT("'%s' Failed to find an Enhanced Input Component! This template is built to use the Enhanced Input system. If you intend to use the legacy system, then you will need to update this C++ file."), *GetNameSafe(this));
	}
}
...
void AM_MCharacter::DoAim(float Yaw, float Pitch)
{
	if (GetController())
	{
...
void AM_MCharacter::DoMove(float Right, float Forward)
{
	if (GetController())
	{
